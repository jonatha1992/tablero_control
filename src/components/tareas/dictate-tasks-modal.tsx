...
import { Mic, Loader2, Send, MessageSquare, Check } from 'lucide-react';
...
function DictateTasksInner({ onTasksConfirmed }: DictateTasksInnerProps) {
  ...
  const { members, locations, projects, cycles, objectives, confirmOptions } =
              ...
              {msg.tasks.length > 0 &&
                (msg.confirmed ? (
                  <div className="flex items-center gap-1 text-xs text-green-600 dark:text-green-400 pl-1">
                    ...
                    {msg.tasks.length} tarea{msg.tasks.length !== 1 ? 's' : ''} creada
                    {msg.tasks.length !== 1 ? 's' : ''}
                  </div>
                ) : (
                  <Button
                    size="sm"
...
export function DictateTasksContent() {
