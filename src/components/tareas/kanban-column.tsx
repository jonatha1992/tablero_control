...
export function KanbanColumn({ status, tasks, onCardClick, onPriorityChange, onLocationChange, onAddClick, selectedTaskIds, isSelectMode, onSelectAll, onBulkDelete, locations }: KanbanColumnProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        'flex flex-col w-[240px] min-w-[240px] max-w-[240px] shrink-0 rounded-md border bg-muted/30 transition-colors h-full',
        isOver && 'bg-primary/5 border-primary/30'
      )}
    >
      <div className="overflow-y-auto flex-1 min-h-0">
        {/* Column header — sticky within scroll */}
        <div className="sticky top-0 z-10 bg-muted/95 backdrop-blur-sm flex items-center justify-between px-2 py-1.5 border-b">
