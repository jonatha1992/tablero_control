...
- Name: tareas.spec.ts >> Gestión de Tareas — Kanban >> botón FAB "+" existe para crear tarea rápida
- Location: tests\tareas.spec.ts:130:7
...
Locator: locator('button.fixed.bottom-10, button[class*="fixed"][class*="rounded-full"]').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('button.fixed.bottom-10, button[class*="fixed"][class*="rounded-full"]').first()
